...
func (h *PublicHeader) Write(b *bytes.Buffer, version protocol.VersionNumber, pers protocol.Perspective) error {
	...
	case protocol.PacketNumberLen6:
		utils.GetByteOrder(version).WriteUint48(b, uint64(h.PacketNumber)&0xFFFFFFFFFFFF)
	default:
		return errPacketNumberLenNotSet
	}
...
// ParseConnectionID parses the connection ID from a QUIC packet's public header.
// It no error occurs, it restores the read position in the bytes.Reader.
func ParseConnectionID(b *bytes.Reader, packetSentBy protocol.Perspective) (protocol.ConnectionID, error) {
